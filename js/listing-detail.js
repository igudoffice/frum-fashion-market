...
function getContactMarkup(listing) {
...
function renderListingDetail() {
  ...
  document.getElementById('detailContent').innerHTML = `
    <div class="detail-gallery">
      <div class="detail-main-image" style="${getPlaceholderStyle(listing)}">
        ${createListingVisualMarkup(listing, 'detail')}
      </div>
    </div>
    <div class="detail-info">
      ...
      <div class="detail-seller">
        ...
        <div class="seller-location">📍 ${formatLocation(listing.location)}</div>
        ${getContactMarkup(listing)}
      </div>
    </div>
  `;
...
function loadRelatedListings(listing) {
  ...
  if (related.length > 0) {
    ...
    related.forEach(item => {
      grid.appendChild(createListingCard(item, { showPosted: false }));
    });
  }
}
